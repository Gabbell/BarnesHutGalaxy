...
QuadTreeNode::QuadTreeNode(int nodeId, const float3& min, const float3& max, QuadTreeNode* parent)
	: _id(nodeId)
	, _min(min)
	, _max(max)
	, _center(make_float3(min.x + (max.x - min.x) / 2.0f, min.y + (max.y - min.y) / 2.0f, 0.0f))
	, _parent(parent)
	, _star(nullptr)
	, _numParticles(0)
{
...
QuadTreeNode* QuadTreeNode::createQuadNode(Quadrant quad)
{
	switch (quad)
	{
		...
		case Quadrant::NW: return new QuadTreeNode(++s_idCounter, make_float3(_min.x, _center.y, 0.0),
			make_float3(_center.x, _max.y, 0.0),
			this);
		...
		case Quadrant::SE: return new QuadTreeNode(++s_idCounter,
			make_float3(_center.x, _min.y, 0.0),
			make_float3(_max.x, _center.y, 0.0),
			this);
		default:
		{
			throw std::runtime_error("Can't determine quadrant");
...
void QuadTreeNode::getQuadTreeVerts(std::vector<float3>& quads)
{
	quads.push_back(make_float3( _min.x, _min.y, 0.0f ));
	quads.push_back(make_float3( _min.x, _max.y, 0.0f ));
	quads.push_back(make_float3( _max.x, _max.y, 0.0f ));
	quads.push_back(make_float3( _max.x, _min.y, 0.0f ));
...
void QuadTreeNode::computeMassDistribution()
{
	...
	{
		_mass = 0;
		_centerOfMass = make_float3(0.0f, 0.0f, 0.0f);
...
float3 QuadTreeNode::calcAcc(const Star* const star1, const Star* const star2)
{
	float3 acc;
	...
	float3 difference = star2->position - star1->position;
...
float3 QuadTreeNode::calcTreeForce(const Star* const star)
{
	...
	{
		float3 difference = _centerOfMass - star->position;
		double distance = length(difference) + SOFTENER;
...
void QuadTreeNode::traverseAndPopulate(Node* toGpu)
{
	...
	toGpu[nodeIndex].centerOfMass = _centerOfMass;
